...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        value = value.replace('+', ' ')
        print(value)
        tokenizer = self.convai.tokenizer
        personality = self.convai.personality
        model = self.convai.model
        args = self.convai.args
        history = self.history
        history.append(tokenizer.encode(value))
        with torch.no_grad():
...
class http_server:
    def __init__(self, convai, ip, port):
...
def server_run(ip,port):
    convai = ConvAI()
    print("Starting server","ip: "+ ip, "Port: "+ str(port),sep="\n")
    myserver = http_server(convai=convai, ip=ip, port=port)
